...
def run_experiment(_config: DictConfig) -> float:
    ...
    # )
    learner_keys = []
    for i in range(config.system.num_learners):
        # PRNG keys.
        key, key_e, wm_key, actor_net_key, critic_net_key = jax.random.split(
            jax.random.PRNGKey(config.arch.seed), num=5
        )
        learner_keys.append((key, key_e, wm_key, actor_net_key, critic_net_key))
    ...
    learners = []
    for i in range(config.system.num_learners):
        # Each learner gets a different set of keys
        key, key_e, wm_key, actor_net_key, critic_net_key = learner_keys[i]
        learn, root_fn, search_apply_fn, learner_state = learner_setup(
            env, (key, wm_key, actor_net_key, critic_net_key), config
        )
        learners.append((learn, root_fn, search_apply_fn, learner_state))
    ...
    for eval_step in range(config.arch.num_evaluation):
        ...
        selected_learner_idx = random.randint(0, config.system.num_learners - 1)
        learn, root_fn, search_apply_fn, learner_state = learners[selected_learner_idx]
        ...
        for i in range(config.system.num_learners):
            learners[i] = (
                learn, root_fn, search_apply_fn, learner_output.learner_state
            )
            # l, r, s, _ = learners[i]
            # learners[i] = (l, r, s, learner_output.learner_state)
            # learners[i] = learners[i][:-1] + (learner_output.learner_state,)
